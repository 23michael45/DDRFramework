...
static LONG findDrvPath (char *clsidstr,char *dllpath,int dllpathsize)
{
	...
	mbstowcs(wclsidstr, clsidstr, strlen(clsidstr) + 1);//Plus null

	CharLowerBuff(wclsidstr,strlen(clsidstr));
	if ((cr = RegOpenKey(HKEY_CLASSES_ROOT,COM_CLSID,&hkEnum)) == ERROR_SUCCESS) {
		...
		while (cr == ERROR_SUCCESS && !found) {
			...
			if (cr == ERROR_SUCCESS) {
				...
				wchar_t wdatabuf[128];
				mbstowcs(wdatabuf, databuf, strlen(databuf) + 1);//Plus null
				CharLowerBuff(wdatabuf,strlen(databuf));
